...
var add_axis, add_bars, add_labels, add_legend, add_states, add_to_layers, add_to_series, add_tooltip, bar_chart, bar_x, bar_y, bring_to_top, calculate_percentages, calculate_scores, change_green_val, chart_colors, chart_group, chart_x_scale, clicked_outside, color_scale, compare_click, compare_states_mode, default_click, empty_hash_series, ensure_proper_size, explain, explanation_tooltip, fill_color, focus_bars, get_absolute, get_chart_data, get_focused, get_labels, get_x_scale, height, hide_tooltip, highlight_bars, initialize_layers, legend_positions, make_chart, make_hash_series, margins, over_time, rbar, rbox, regular_state_mode, remove_tooltips, rlabel, rlayer, rscale, setup_chart, share_opinion, show_map, show_tooltip, state_hover, state_unhover, states_fade_in, stream_data, streamgraph, title, title_string, to_compare, toggle_controls, toggle_sliders, tooltip_says, tooltips, translations, unhighlight_bars, update_bars, width, x_label, _index1, _index2, _max, _min, _percentages, _scores, _totals;
...
explain = function() {
  return explanation_tooltip.style('vizibility', 'true').style('left', "" + d3.event.clientX + "px").style('top', "" + d3.event.clientY + "px");
};
...
make_chart = function(national) {
  ...
  if (to_compare.length || national) {
    ...
    chart_group.append('text').style('text-anchor', 'middle').style('x', width / 2).style('y', height / 2).text("Select states to compare");
  }
