...
$(function() {
  ...
  $('#temporal_mode').on('click', temporal_mode);
  $('#static_mode').hide();
  return $('#static_mode').on('click', static_mode);
});
...
temporal_mode = function() {
  var state, values, _i, _len, _ref, _results;
...
fill_color = function(state) {
  ...
  if (over_time) {
    gradient = (_scores[state][_index1] - _scores[state][_index2]) / _scores[state][_index1];
    if (gradient > 0) {
...
streamgraph = function() {
  var area, layers, stack, stream_x, stream_y;

  stack = d3.layout.stack().offset('wiggle').values(function(d) {
    return d.values;
  });
  layers = stack(stream_data());
  stream_x = d3.scale.linear().domain([_max, _min]).range([margins['left'], width - margins['right']]);
