...
show_map = function(comparing) {
  ...
  if (!comparing) {
    console.log('unovering every state');
    for (_i = 0, _len = to_compare.length; _i < _len; _i++) {
      state = to_compare[_i];
      state_unhover(state);
    }
...
clicked_outside = function(event) {
  ...
  id = event.target.id;
  if (!(rlayer.test(id) || rlabel.test(id) || rbox.test(id) || rbar.test(id) || rscale.test(id) || id.length === 2 || id === 'make_chart' || id === 'replay')) {
    return show_map();
  }
};
