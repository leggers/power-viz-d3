...
$(function() {
  ...
  return $('#controls_toggle').on('click', toggle_controls);
});
...
bar_y = function(bar_id) {
  ...
  return (height - margins['top']) * bar_num / num_bars;
};
...
streamgraph = function() {
  ...
  stack = d3.layout.stack().offset('wiggle').values(function(d) {
    return d.values;
  });
  layers = stack(stream_data());
  stream_x = d3.scale.linear().domain([_min, _max]).range([margins['left'], width - margins['right']]);
  ...
  chart_group.append('g').attr('class', 'axis').attr('transform', "translate(" + (margins['left'] - 5) + ", 0)").call(y_axis);
  chart_group.append('text').attr("transform", "rotate(-90)").attr('y', -margins['left']).attr('x', height / 2 + 50).attr('dy', '1em').attr('class', 'stream_y_label').text("Gigawatt-Hours");
  ...
  return chart_group.selectAll('.layer').attr('opacity', 1).on('mouseover', function(d, i) {
    d3.selectAll('#veil').transition().duration(200).attr('opacity', .9);
    return chart_group.selectAll('.layer').transition().duration(200).attr('opacity', function(d, j) {
      if (j !== i) {
        return .3;
      }
    });
  }).on('mouseleave', function() {
    d3.selectAll('#veil').transition().duration(200).attr('opacity', .75);
    return chart_group.selectAll('.layer').transition().duration(200).attr('opacity', 1);
  });
};
