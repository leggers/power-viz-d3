...
var add_bars, add_state, bring_to_top, calculate_percentages, calculate_scores, change_green_val, fill_color, get_chart_data, green_scale, height, make_chart, margins, pie_colors, red_scale, setup_chart, show_map, update_bars, width, _index, _max, _min, _percentages, _scores, _totals;
...
show_map = function() {
  return d3.selectAll('#chart_group').selectAll('g').remove();
};
...
setup_chart = function() {
  d3.select('#chart_group').append('rect').attr('id', 'veil').attr('width', '959').attr('height', '593').attr('fill', 'white').attr('opacity', '0.75');
  return d3.select('#chart_group').append('div').attr('class', 'tooltip');
};

add_state = function(state_id) {
  return d3.select('#chart_group').append('g').attr('id', "" + state_id + "_bars").attr('transform', "translate: " + (width / 2) + ", " + (height / 2));
};

add_bars = function(state_id) {
  var data, labels, numbers;
  ...
  numbers = data[1];
  console.log(numbers);
  return d3.select("#" + state_id + "_bars").selectAll('rect').data(numbers).enter().append('rect').attr('x', function(d, i) {
    return 20 * i;
  }).attr('y', function(d, i) {
    return 0;
  }).attr('width', 18).attr('height', function(d, i) {
    return d || 0;
...
update_bars = function() {
  return d3.select('#chart_group').selectAll('g').selectAll('rect');
};
